...
const test_db = (options, callback) => {
  // query
  ...
  if (Object.keys(options).length === 0) {
    ...
    query.map(item => {
      if (item === "name") {
        sql += `select idMovie, c00, c08, c20 from movie where c00 like '%${options[item]}%' order by idMovie desc`;
      }
      ...
      if (item === "limit") {
        if (sql === "") {
          sql += `select idMovie, c00, c08, c20, premiered, rating from movie_view order by premiered desc limit ${options[item]}`;
        } else {
      ...
      if (item === "offset") {
        if (sql === "") {
          sql += `select idMovie, c00, c08, c20, premiered, rating from movie_view order by premiered desc offset ${options[item]}`;
        } else {
    ...
    db.all(sql, [], (err, movies) => {
      if (err) {
        ...
        if (Object.keys(movies).length === 0) {
          ...
          console.log("return movies with callback function");
          callback(err, movies);
          // console.log(movies);
